                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-12 mb-3">
                                <label for="image">Pilih Icon Service</label>
                                <select name="image" id="image" class="form-select" onchange="previewImage()">
                                    <option value="">Pilih Icon</option>
                                    @for ($i = 1; $i <= 48; $i++)
                                        <option value="{{ $i }}.png"
                                            {{ old('image', $service->image) == $i.'.png' ? 'selected' : '' }}>
                                            Icon {{ $i }}
                                        </option>
                                    @endfor
                                </select>
                                ...
                                <div class="mt-3 text-center">
                                    <img id="preview-img" 
                                        src="{{ $service->image ? asset('assets/images/service/' . $service->image) : '' }}" 
                                        alt="Preview" 
                                        style="max-height:100px; display: {{ $service->image ? 'block' : 'none' }}; margin:auto;">
                                </div>
                            </div>
                            ...
                            <div class="col-12 mb-3">
                                <x-form.text label="Nama Service (ID)" for="title_id" name="title_id"
                                    value="{{ old('title_id', $service->title_id) }}" :error="$errors->first('title_id')" />
                            </div>

                            {{-- Title English --}}
                            <div class="col-12 mb-3">
                                <x-form.text label="Nama Service (EN)" for="title_en" name="title_en"
                                    value="{{ old('title_en', $service->title_en) }}" :error="$errors->first('title_en')" />
                            </div>

                            {{-- Description Indonesia --}}
                            <div class="col-12 mb-3">
                                <label>Deskripsi (ID)</label>
                                <textarea name="description_id" class="form-control" rows="4">{{ old('description_id', $service->description_id) }}</textarea>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            <label>Status</label>
                            {{ Form::select('status', ['0' => 'Inactive', '1' => 'Active'], old('status', $service->status), ['class' => 'form-select']) }}
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="datetime-local" name="created_at" class="form-control"
                                value="{{ old('created_at', $service->created_at ? $service->created_at->format('Y-m-d\TH:i') : '') }}">
