...
@section('content')
<div class="container">
    <form method="POST" action="{{ url('data/media',$data->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <x-form.date label="Created At" for="created_at" name="created_at"
                                    value="{{ $data->created_at ? date('Y-m-d', strtotime($data->created_at)) : '' }}" 
                                    :error="$errors->first('created_at')" required />
                            </div>
                            <div class="col-12">
                                <x-form.text label="Nama Media" for="title" name="title"
                                    value="{{ old('title', $data->title) }}" :error="$errors->first('title')" required></x-form.text>
                            </div>
                            <div class="col-md-6">
                                <x-form.text label="penulis" for="penulis" name="penulis"
                                    value="{{ old('penulis',$data->penulis) }}" :error="$errors->first('penulis')" required></x-form.text>
                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <x-form.text label="caption" for="caption" name="caption"
                                value="{{ old('caption', $data->caption) }}" :error="$errors->first('caption')" required></x-form.text>
                        </div>
